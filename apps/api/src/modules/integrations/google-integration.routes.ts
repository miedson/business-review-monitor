...
const googleLocationsRouteSchema = {
  ...
  description: "Lists Google Business Profile locations accessible for a connected account.",
  security: [{ bearerAuth: [] }],
  querystring: {
    type: "object",
    required: ["accountId"],
...
export function registerGoogleIntegrationRoutes(
  ...
  app.get(
    ...
    { schema: googleConnectRouteSchema },
    async (request, reply) => {
  ...
  app.get(
    ...
    { schema: googleAccountsRouteSchema },
    async (request, reply) => {
      const userId = await getAuthenticatedUserId(request);
      const session = await options.authService.getCurrentSession(userId);
  ...
  app.post(
    "/reviews/:reviewId/reply",
    {
      schema: {
        ...
        body: {
          type: "object",
          required: ["accountId", "locationId", "message"],
          properties: {
            accountId: { type: "string" },
    ...
    async (request, reply) => {
      ...
      const body = z
        ...
        .parse(request.body);
      const location = await options.businessLocationRepository.findByGoogleIds({
        tenantId: session.tenant.id,
        googleAccountId: body.accountId,
        googleLocationId: body.locationId,
      });
      if (!location?.isActive)
        return reply
          .status(404)
          .send({ error: "Google location not found", requestId: request.id });
      ...
      if (!connection?.encryptedRefreshToken || connection.status !== "CONNECTED")
        return reply
          .status(401)
          .send({ error: "Google connection is required", requestId: request.id });
      const refreshToken = options.tokenCipher.decrypt(connection.encryptedRefreshToken);
      const tokenSet = await options.googleProvider.refreshAccessToken({ refreshToken });
      if (!options.googleProvider.replyToReview)
        return reply
          .status(501)
          .send({ error: "Google reply is not supported by this provider", requestId: request.id });
      const reviewId = (request.params as { reviewId: string }).reviewId;
      await options.googleProvider.replyToReview({
      ...
      await options.reviewCacheRepository.saveReply?.({
        tenantId: session.tenant.id,
      ...
      await options.realtimeGateway.publish({
        tenantId: session.tenant.id,
        type: "google.review.replied",
        payload: { reviewId, locationId: body.locationId },
      });
...
function mapProviderErrorStatus(
  ...
  if (error.code === "GOOGLE_INVALID_CALLBACK" || error.code === "GOOGLE_INVALID_STATE") {
    return 400;
  }

  if (error.code === "GOOGLE_AUTH_REQUIRED" || error.code === "GOOGLE_TOKEN_REVOKED") {
    return 401;
  }
