...
export function registerInstagramCommentsRoutes(
  ...
  app.get<{
    ...
    "/instagram/comments",
    { schema: listInstagramCommentsRouteSchema },
    async (request, reply) => {
      ...
      try {
        ...
        if (query.instagramConnectionId)
          executeInput.instagramConnectionId = query.instagramConnectionId;
        ...
        const accessToken = connection?.encryptedAccessToken
          ? options.tokenCipher.decrypt(connection.encryptedAccessToken)
          : null;
        const mediaIds = [
          ...new Set(
            result.comments
              .map((comment) => comment.externalMediaId)
        ...
        const authorIds = [
          ...new Set(
            result.comments
              .map((comment) => comment.authorExternalId)
              .filter((id): id is string => Boolean(id)),
          ),
        ];
        const [mediaEntries, userEntries] = accessToken
          ? await Promise.all([
              options.instagramProvider.getMediaMetadata
                ? Promise.all(
                    mediaIds.map(
                      async (id) =>
                        [
                          id,
                          await options.instagramProvider
                            .getMediaMetadata?.(accessToken, id)
                            .catch(() => null),
                        ] as const,
                    ),
                  )
                : Promise.resolve([]),
              options.instagramProvider.getExternalUserProfile
                ? Promise.all(
                    authorIds.map(
                      async (id) =>
                        [
                          id,
                          await options.instagramProvider
                            .getExternalUserProfile?.(accessToken, id)
                            .catch(() => null),
                        ] as const,
                    ),
                  )
        ...
        const comments = result.comments.map((comment) => ({
          ...
          author: {
            ...
            profilePictureUrl: comment.authorExternalId
              ? (userById.get(comment.authorExternalId)?.profile_pic ?? null)
              : null,
            authorType:
              comment.authorUsername === connection?.username ||
              comment.authorExternalId === connection?.instagramProfessionalAccountId
                ? "BUSINESS"
                : "CUSTOMER",
          },
    ...
    "/instagram/comments/:id/reply",
    {
      schema: {
        ...
        body: {
          type: "object",
          required: ["message"],
          properties: { message: { type: "string", minLength: 1, maxLength: 2200 } },
        },
      },
    },
    async (request, reply) => {
      ...
      const comment = await options.instagramCommentRepository.findByIdForTenant({
        id: request.params.id,
        tenantId: session.tenant.id,
      });
      if (!comment)
        return reply.status(404).send({ error: "Comment not found", requestId: request.id });
      const connection = await options.instagramConnectionRepository.findByTenantId(
        session.tenant.id,
      );
      if (!connection?.encryptedAccessToken || connection.status !== "CONNECTED")
        return reply
          .status(401)
          .send({ error: "Instagram connection is required", requestId: request.id });
      const accessToken = options.tokenCipher.decrypt(connection.encryptedAccessToken);
      if (!options.instagramProvider.replyToComment)
        return reply.status(501).send({
          error: "Instagram reply is not supported by this provider",
          requestId: request.id,
        });
      try {
        const result = await options.instagramProvider.replyToComment({
          accessToken,
          commentId: comment.externalCommentId,
          message: body.message,
        });
        await options.instagramCommentRepository.saveReply?.({
          tenantId: session.tenant.id,
        ...
        await options.instagramCommentRepository.markReplied?.({
          id: comment.id,
          tenantId: session.tenant.id,
          repliedAt: new Date(),
        });
        await options.realtimeGateway.publish({
          tenantId: session.tenant.id,
          type: "instagram.comment.replied",
        ...
        if (error instanceof GoogleBusinessProfileProviderError) {
          if (isUnavailableInstagramCommentError(error)) {
            return reply.status(409).send({
              error:
                "Este comentário não está mais disponível no Instagram e não pode receber resposta.",
              code: "INSTAGRAM_COMMENT_UNAVAILABLE",
              requestId: request.id,
            });
          }
          const status =
            error.code === "INSTAGRAM_AUTH_REQUIRED" || error.code === "INSTAGRAM_TOKEN_REVOKED"
              ? 401
  ...
  app.post<{ Params: { id: string } }>(
    "/instagram/comments/:id/mark-replied",
    {
      schema: {
        tags: ["Instagram Comments"],
        summary: "Manually mark an Instagram comment as replied",
        security: [{ bearerAuth: [] }],
        params: { type: "object", required: ["id"], properties: { id: { type: "string" } } },
      },
    },
...
function isUnavailableInstagramCommentError(error: unknown): boolean {
  ...
  const details = cause as {
    httpStatus?: unknown;
    metaError?: { code?: unknown; error_subcode?: unknown };
  };
  return (
    details.httpStatus === 400 &&
    details.metaError?.code === 100 &&
    details.metaError.error_subcode === 33
  );
}
