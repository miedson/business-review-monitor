...
export function registerRealtimeRoute(app: FastifyInstance, input: { gateway: RealtimeGateway; webUrl: string; authService: { getCurrentSession: (userId: string) => Promise<{ tenant: { id: string } }> } }): void {
  app.get("/realtime", { schema: { tags: ["Realtime"], summary: "Open tenant realtime stream", security: [{ bearerAuth: [] }] } }, async (request, reply) => {
    ...
    const corsOrigin = origin && origin === input.webUrl.replace(/\/$/, "") ? origin : input.webUrl.replace(/\/$/, "");
    response.writeHead(200, { "content-type": "text/event-stream; charset=utf-8", "cache-control": "no-cache, no-transform", connection: "keep-alive", "x-accel-buffering": "no", "access-control-allow-origin": corsOrigin, "access-control-allow-credentials": "true", vary: "Origin" }); response.write(": connected\n\n");
