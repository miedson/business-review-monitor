...
export function registerAttentionSummaryRoute(
  ...
  app.get("/attention-summary", { schema: routeSchema }, async (request, reply) => {
    ...
    const [googleReviewsPendingReply, instagramCommentsPendingReply] = await Promise.all([
      input.prisma.reviewCache.count({
        where: {
      ...
      input.prisma.instagramComment.count({
        where: { tenantId: session.tenant.id, repliedAt: null },
      }),
    ]);
    return reply.send({
      googleReviewsPendingReply,
      instagramCommentsPendingReply,
      total: googleReviewsPendingReply + instagramCommentsPendingReply,
    });
  });
}
