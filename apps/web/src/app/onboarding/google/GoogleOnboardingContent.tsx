...
import { Box, Button, Text, Flex, Badge, Alert } from "@/lib/design-system";
  ...
  return (
    <Box css={{ minH: "100vh", bg: "surface.secondary", px: 4, py: 8 }}>
      <Box css={{ maxW: "600px", mx: "auto" }}>
        ...
        <Text color="text.tertiary" textAlign="center" mb={8} maxW="500px" mx="auto" lineHeight="relaxed">
          Conecte seu perfil para acompanhar avaliações e manter sua
          reputação centralizada no Business Reputation Hub.
        </Text>
        ...
        <Box css={{ bg: "surface.primary", border: "1px solid", borderColor: "surface.border", borderRadius: "2xl", boxShadow: "sm", p: { base: 5, md: 6 } }}>
          <Box css={{ display: "flex", alignItems: "flexStart", gap: 4, mb: 6 }}>
            ...
            <Box css={{ flex: 1 }}>
              <Text fontSize="lg" fontWeight="semibold" color="text.primary">Google Business Profile</Text>
              <Text fontSize="sm" color="text.tertiary">Avaliações, nota da empresa, novos feedbacks</Text>
            </Box>
          </Box>

          {error && <Alert tone="error" onClose={() => setError(null)} dismissible>{error}</Alert>}

          {connected ? (
            <Box css={{ display: "flex", alignItems: "center", gap: 3, p: 4, bg: "status.success.bg", border: "1px solid", borderColor: "status.success.border", borderRadius: "lg" }}>
              <Badge variant="success" size="md" dot>Conectado</Badge>
              {accountName && <Text fontSize="sm" color="text.secondary">Conta: {accountName}</Text>}
            </Box>
          ) : (
            <Button
              variant="solid"
        ...
        <Flex css={{ justifyContent: "center", gap: 3, mt: 8, flexWrap: "wrap" }}>
          ...
          {connected && <Button size="md" onClick={handleContinue}>Continuar</Button>}
        </Flex>
      </Box>
    </Box>
  );
