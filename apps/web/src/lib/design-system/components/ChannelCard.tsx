...
import { Badge, Box, Button, ConfirmDialog, Text } from "@/lib/design-system";
...
const providerConfig: Record<
  ChannelProvider,
  { icon: ReactNode; color: string; background: string; foreground: string }
> = {
  google: {
    icon: (
    ...
    return (
      <>
        ...
        >
          ...
          >
            ...
            <Box css={{ flex: 1, minWidth: 0 }}>
              <Box css={{ display: "flex", alignItems: "center", gap: 2, flexWrap: "wrap" }}>
                ...
                <Badge
                  variant="subtle"
                  colorScheme={statusVariants[status] as "success" | "default" | "info" | "warning"}
                  size="sm"
                >
                  {statusLabels[status]}
                </Badge>
                {accountLabel && (
                  <Badge variant="subtle" colorScheme="slate" size="sm">
                    {accountLabel}
                  </Badge>
                )}
              </Box>
              {subtitle && (
                <Text css={{ fontSize: "xs", color: "text.tertiary", lineHeight: "normal", mt: 1 }}>
                  {subtitle}
                </Text>
              )}
            </Box>
          ...
          {comingSoon && (
            <Text
              css={{ mt: 2, ml: { base: 0, md: 11 }, fontSize: "xs", color: "text.quaternary" }}
            >
              Em breve
            </Text>
          )}
